...
Dose-Response-Optim-Boot-Enhancement.py is a modified script of Dose-Response-Optim-Boot.py that automates optimization of multiple datasets for all specified dose-response functions.

@author: Mark H. Weir Ph.D. - weir.95@osu.edu
"""

"""Include the packages you will need""" 
import numpy as np
import pandas as pd
...
def run_dataset(
    ...
    print(f'=====Results from Optimization for {modeltofit}=============================================================')
...
if __name__ == '__main__':
    ...
    dir_datasets = Path.home() / "Desktop/Publications in progress/Candida DR"
    dir_datasets.mkdir(parents=True, exist_ok=True)
    file_datasets_todo = dir_datasets /'candida_dr_data_master_all.csv'
    ...
    for idx, row in per_strain_list_df.iterrows():
        ...
        dir_dataset = dir_datasets / 'autodata' / f"{row['Source']}_{row['Species']}_{row['Strain']}".replace(' ', '-').lower()
        dir_dataset.mkdir(parents=True, exist_ok=True)
        
        for modeltofit in models_to_fit:
            try:
                dataset_name = f"{row['Source']}_{row['Species']}_{row['Strain']}_{modeltofit}".replace(' ', '-').lower()
                if dataset_name in datasets_done:
                    continue
                bootstrap_parameters = run_dataset(
                    df_dataset,
                    modeltofit=modeltofit,
